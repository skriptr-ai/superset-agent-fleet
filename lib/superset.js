...
import { call, callDirectOnly, runCli, useDirect, transportNote } from './transport.js';
...
export async function localHostId() {
  ...
  // Asked of the machine's OWN WORKSPACES rather than of `superset status`, because status is
  // the one command that gets this wrong outside an interactive session. Run under launchd's
  // environment it reports an unregistered id and a null name — a machine that does not know
  // it is itself — while `workspaces list`, `hosts list` and the host service all resolve
  // correctly in that same environment. No variable restores it; passing every `SUPERSET_*`
  // the shell has changes nothing.
  //
  // This is also the answer that cannot disagree with the question. The id exists to be
  // compared against `ws.hostId` on rows from this same call, so taking it FROM those rows
  // makes the comparison self-consistent by construction. Deriving it separately is what let
  // the two drift apart and badge every agent on this machine as living somewhere else.
  ...
  localHost = rows.find((w) => typeof w?.hostId === 'string')?.hostId ?? null;

  // A machine with no workspaces at all has no row to read, and then a wrong answer still
  // beats none: it names SOME host, so at worst the badges are wrong rather than the fleet.
  if (!localHost) {
    const status = await runCli(['status']);
    if (status.ok && typeof status.data?.hostId === 'string') localHost = status.data.hostId;
  }

  // The name lives only in the host list, which is cached and correct in every environment.
  if (localHost) {
    const known = await listHosts();
    localHostName = known.hosts.find((h) => h.id === localHost)?.name ?? null;
  }
  return localHost;
}
