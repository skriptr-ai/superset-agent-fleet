...
export function readConfig(env = process.env) {
  let bind = env.AGENT_FLEET_BIND ?? '127.0.0.1';
  if (bind !== 'localhost' && !isIP(bind)) {
    throw new Error('AGENT_FLEET_BIND must be an IP address or localhost');
  }
  // Normalize IPv6 before checking it: 0:0:0:0:0:0:0:0 is also the wildcard address.
  if (isIP(bind) === 6) bind = new URL(`http://[${bind}]`).hostname.slice(1, -1);
  if (bind === '0.0.0.0' || bind === '::') {
    throw new Error('AGENT_FLEET_BIND must name a specific address, not every network interface');
  }
  if (!['127.0.0.1', '::1', 'localhost'].includes(bind) && !env.AGENT_FLEET_TOKEN) {
    throw new Error('AGENT_FLEET_TOKEN is required when AGENT_FLEET_BIND is outside loopback');
  }
