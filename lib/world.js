...
//
//   1. An orchestrator's own screen, which logs every `superset` command it ran. This is the
...
//   3. The fleet log, when an orchestrator sends through `bin/superset-send` — the one source
//      that is exact rather than inferred, because the orchestrator wrote it as it sent. See
//      lib/fleetlog.js for why a screen can never be enough on its own.
//
...
export class World {
   ...
   */
  constructor(statePath = null, logPath = null) {
  ...
  async poll() {
    ...
    // Read the log for the same window the screens were read in, so both sources describe one
    // tick. It is cheap — a stat, and a read only when the file grew.
    const logged = await this.log.read();
    ...
    const events = this.#diff(agents, logged);
  ...
  #diff(agents, logged = []) {
    ...
    // The logged messages first. They are the exact record of this window's traffic, and a
    // message that is in the log is one the screens were never needed for — the wrapper's own
    // command line is `superset-send …`, which the screen parser does not match, so a message
    // is only ever counted once no matter which source knew about it.
    for (const message of logged) {
      if (!byId.has(message.fromId) || !byId.has(message.toId)) continue;
      this.#recordCall(message.fromId, { verb: message.kind, workspaceId: message.toId });
      push(message.kind, message.fromId, message.toId, message.text, false, message.at);
    }
   ...
   */
    ...
    if (!hubs.size) {
      for (const agent of agents) if (/orchestrat/i.test(agent.name)) hubs.add(agent.id);
    }
