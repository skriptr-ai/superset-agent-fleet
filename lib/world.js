...
import { turnSummary, promptGist } from './activity.js';
...
const MAX_PENDING_EVIDENCE = 1500;
...
/**
 * How far either side of a create's own window a workspace's birth may fall and still be
 * the one it made. The window is read off the orchestrator's transcript on this machine's
 * clock; `createdAt` is stamped by the cloud on its. A few seconds covers the skew between
 * them without reaching the next thing anyone created.
 */
const SPAWN_WINDOW_SLACK_MS = Number(process.env.AGENT_FLEET_SPAWN_SLACK_MS ?? 5000);

/**
 * How long an unresolved create is kept waiting for its workspace to show up in the list.
 * A local one is there on the next tick; one on a slow remote host takes a few polls. Ten
 * minutes is generous, and past it the window is stale evidence about nothing.
 */
const SPAWN_WINDOW_TTL_MS = 10 * 60 * 1000;

/**
 * The workspaces each unresolved create must have made, matched by WHEN they came to exist.
 *
 * A create that printed no id — `ws=$(superset ws create … --quiet)`, `--json > file` — still
 * ran between two timestamps the transcript keeps, and a workspace has a `createdAt`. One
 * born inside that window, by the caller, is the one the call made. The evidence is a
 * coincidence in time rather than a name, so it is held to a strict reading: a workspace two
 * windows could claim — two orchestrators spawning in the same few seconds — goes to neither,
 * because a wrong bartender is worse than an empty stool.
 *
 * @param {{fromId: string, from: number, until: number}[]} windows unresolved creates
 * @param {Map<string, number>} createdAt workspace id → when it came to exist, ms
 * @returns {Map<string, object>} workspace id → the one window that made it
 */
export function spawnsIn(windows, createdAt, slack = SPAWN_WINDOW_SLACK_MS) {
  const out = new Map();
  for (const [id, born] of createdAt) {
    if (!Number.isFinite(born)) continue;
    let match = null;
    let ambiguous = false;
    for (const w of windows) {
      if (w.fromId === id) continue;
      if (born < w.from - slack || born > w.until + slack) continue;
      // Two windows from ONE session — a loop of creates — are one claim; two sessions are two.
      if (match && match.fromId !== w.fromId) ambiguous = true;
      if (!match) match = w;
    }
    if (match && !ambiguous) out.set(id, match);
  }
  return out;
}

/**
 * Whether a create at `at` can have made a workspace born at `born`.
 *
 * A create's output is trusted to name what it made, and the looser it is read the more it
 * can name something else: `superset ws create --help; superset ws list` in one Bash call
 * prints every workspace there is under an `id` key. What a create cannot do is make a
 * workspace that already existed, so a target older than the call is not its spawn — it is
 * something the caller looked at — and is dropped. An unknown birth is let through: the
 * check is a way to refuse a wrong answer, not a way to demand a field.
 */
export function couldHaveMade(at, born, slack = SPAWN_WINDOW_SLACK_MS) {
  if (!Number.isFinite(born) || !Number.isFinite(at) || at <= 0) return true;
  return born >= at - slack;
}

/**
 * A workspace's tags, as Superset files them: one string, comma-separated, each tag also a
 * sidebar folder of that name. An orchestrator that tags its workers has filed them together
 * on purpose, which is the one grouping in the whole fleet that somebody actually meant.
 */
export function tagsOf(ws) {
  const raw = ws?.tags;
...
export class World {
  ...
  constructor(
    ...
    /** @type {Map<string, Map<string, number>>} workspace:terminal → CLI-call signature → times seen */
    ...
    /**
     * @type {Map<string, Set<string>>} workspaceId → peers it has been seen CREATING, a
     * subset of `drives` kept apart because a create is the one command that cannot be
     * mistaken for a report: a worker never creates its coordinator. See #electHubs.
     */
    this.spawned = new Map();
    /**
     * @type {{fromId: string, from: number, until: number, text: string | null,
     *   replay: boolean}[]} creates whose output named no workspace, waiting to be matched
     * by time against the workspaces that appeared while they ran. See spawnsIn.
     */
    this.pendingSpawns = [];
    /** @type {Set<string>} workspaces whose creator a pending spawn has already named */
    this.attributed = new Set();
    /** @type {Map<string, number>} every live workspace → when it came to exist, ms */
    this.createdAt = new Map();
    /** @type {Map<string, string[]>} every live workspace → its tags, the folders it is in */
    this.tagsOf = new Map();
    /**
    ...
    this.hubIds = [];
    /**
     * @type {Set<string>} workspaces a person has put behind the bar by hand.
     *
     * Election needs evidence, and evidence can be missing: an orchestrator that drove one
     * worker at a time, one whose commands scrolled off before the fleet was watched, one
     * running on a machine whose transcripts this instance cannot read. A pin says "I know
     * this one is orchestrating" and is unioned with what was observed. It holds until unpinned,
     * however quiet the session goes, because a person said so and no poll has said otherwise.
     */
    this.pins = new Set();
    /**
     * @type {string | null} the host this process runs on. Discovery can correct a provisional
     * identity after the first workspace appears.
     */
    this.localHostId = null;
    /** @type {Map<string, number>} workspaceId → poll ordinal of its last screen read */
    this.readAt = new Map();
  ...
  async load() {
    ...
    try {
      ...
      for (const [id, peers] of Object.entries(saved.spawned ?? {})) {
        this.spawned.set(id, new Set(peers));
      }
      const hubs = saved.hubIds ?? (saved.hubId ? [saved.hubId] : []);
      this.hubIds = hubs.filter(Boolean);
      this.pins = new Set((saved.pins ?? []).filter(Boolean));
    } catch {
  ...
  async poll() {
    this.tick += 1;
    // A status-only identity from empty startup is provisional and must be rechecked.
    this.localHostId = await this.source.localHostId();
    this.readErrors = [];
    const discovery = await this.source.listWorkspaces({ scope: this.scope });
    const { ok, error, workspaces, knownWorkspaces = workspaces } = discovery;
    if (!ok && !discovery.hostListComplete) {
      const agents = [...this.previous.values()].map((before) =>
        this.#stale(before, error || 'Workspace discovery unavailable'),
      );
      this.previous = new Map(agents.map((agent) => [agent.id, agent]));
      return this.#snapshot({
        error: error || 'Workspace discovery unavailable',
        agents,
        events: [],
      });
    }
    this.unavailable = new Set(discovery.incompleteHostIds ?? []);
    const authoritative = new Set(discovery.authoritativeHostIds ?? []);
    if (this.scope === 'host' && authoritative.size === 1) {
      this.localHostId = [...authoritative][0];
    }
    ...
    const evidence = knownWorkspaces.filter((w) => !w.archivedAt && w.worktreeExists !== false);
    this.knownIds = new Set(evidence.map((w) => w.id));
    // When each came to exist and which folders it is filed in, fleet-wide: a create here
    // can have made a workspace there, and a folder can span hosts.
    this.createdAt = new Map(evidence.map((w) => [w.id, Date.parse(w.createdAt)]));
    this.tagsOf = new Map(evidence.map((w) => [w.id, tagsOf(w)]));
    ...
    this.bindings = await this.source.listTerminalAgents();
    ...
    // Failed/offline hosts have not authoritatively removed their workspaces.
    const present = new Set(agents.map((agent) => agent.id));
    for (const before of this.previous.values()) {
      if (
        !present.has(before.id) &&
        (this.unavailable.has(before.hostId) ||
          (discovery.hostListComplete === false && !authoritative.has(before.hostId)))
      ) {
        agents.push(this.#stale(before, 'Host is unavailable'));
      }
    }
    ...
    // What each agent is working on and what it is doing about it right now, from the same
    // transcripts where they exist and from the screen where they do not. The Superset task
    // is asked for by id and remembered, so this costs a cloud call per NEW task, not per tick.
    const tasks = await this.source.lookupTasks(
      agents.filter((a) => !a.stale).map((a) => a.taskId),
    );
    for (const agent of agents) if (!agent.stale) this.#describe(agent, tasks.get(agent.taskId));
    ...
    const problems = [error, ...this.readErrors].filter(Boolean);
    return this.#snapshot({ error: problems.length ? problems.join('; ') : null, agents, events });
  }
  ...
  #planReads(live) {
    ...
    for (const ws of live) {
      ...
      if (
        this.#isLocal(ws.hostId) ||
        unknown ||
        before?.stale ||
        (before && before.status !== 'idle' && before.status !== 'exited')
      ) {
        reading.add(ws.id);
        continue;
      }
    ...
    // Poll order stays fair even when several fast polls complete in the same millisecond.
    for (const id of reading) this.readAt.set(id, this.tick);
    return reading;
  }
  ...
  #snapshot({ error, agents, events }) {
    return {
      ...
      pins: [...this.pins],
      scope: this.scope,
      hostName: this.source.localHostNameOf(),
      agents,
      events,
  ...
  #stale(before, error) {
    return { ...before, stale: true, readError: error };
  }

  #failedWorkspace(ws, error) {
    const message = `${ws.name || 'Workspace'}: ${error || 'terminal unavailable'}`;
    this.readErrors.push(message);
    const before = this.previous.get(ws.id);
    return before ? this.#stale(before, message) : null;
  }
  ...
  async #readWorkspace(ws) {
    ...
    const listed = await this.source.listTerminals(ws.id, ws.hostId);
    if (!listed.ok) return this.#failedWorkspace(ws, listed.error);
    const { sessions } = listed;
    const terminals = await mapLimit(sessions, READ_CONCURRENCY, async (session) => {
      // Exited PTYs can no longer supply a screen. Their explicit lifecycle state must
      // not freeze a healthy sibling terminal behind a failed observation.
      const screen = session.exited
        ? { ok: true, text: '' }
        : await this.source.readTerminal(ws.id, session.terminalId, ws.hostId);
      if (!screen.ok) return { readFailure: screen.error || 'Terminal read unavailable' };
      const parsed = classifyTerminal({ title: session.title, text: screen.text });
      const merged = mergeStatus(
        parsed,
        this.#isLocal(ws.hostId) ? this.bindings.get(session.terminalId) : undefined,
      );
      return {
    ...
    const failed = terminals.find((terminal) => terminal.readFailure);
    if (failed) return this.#failedWorkspace(ws, failed.readFailure);
  ...
  /**
   * The task an agent is on, and the phase it is in — the two lines a person wants at a glance
   * ahead of whatever the agent last happened to say.
   *
   * The task's title is taken from the best source that has one. Claude's own title for the
   * session comes first: it is generated from the actual exchange, so a workspace called
   * `pt-666` reads as what PT-666 turned out to be about. The Superset task's title is next,
   * and it is the only one available for a Codex agent or a session on another machine. The
   * opening of the prompt the agent is acting on is last, and is kept alongside either way,
   * because the current instruction is often narrower than the session it arrived in.
   *
   * The phase is read off the tool calls of the current turn: the transcript's when this
   * machine has it, the screen's otherwise, and the record says which.
   */
  #describe(agent, task) {
    const session =
      agent.flavor !== 'codex' && this.#isLocal(agent.hostId)
        ? this.transcripts.sessionOf(agent.id)
        : null;
    const lead = agent.terminals.find((t) => t.id === agent.leadTerminalId) ?? agent.terminals[0];
    const recorded = Boolean(session);
    const tools = recorded ? session.tools : (lead?.tools ?? []);
    agent.doing = { ...turnSummary(tools), source: recorded ? 'transcript' : 'screen' };

    const title = session?.title ?? task?.title ?? promptGist(session?.prompt) ?? null;
    agent.task = {
      title,
      source: session?.title ? 'session' : task?.title ? 'task' : title ? 'prompt' : null,
      slug: task?.slug ?? null,
      ask: promptGist(session?.prompt, 220),
      askAt: session?.promptAt ?? null,
    };
  }
  ...
  #newCalls(terminal, workspaceId) {
    ...
    const key = `${workspaceId}:${terminal.id}`;
    const before = this.callCounts.get(key) ?? new Map();
    this.callCounts.set(key, now);
  ...
  #deferEvidence(message) {
    const queuedAt = message.queuedAt ?? Date.now();
    if (Date.now() - queuedAt > EVIDENCE_TTL_MS) return;
    this.pendingEvidence.push({ ...message, queuedAt });
    if (this.pendingEvidence.length > MAX_PENDING_EVIDENCE) this.pendingEvidence.shift();
  }
  ...
  #diff(agents, recorded = []) {
    ...
    const retry = this.pendingEvidence;
    this.pendingEvidence = [];
    for (const message of [...retry, ...recorded]) {
      if (message.queuedAt && at - message.queuedAt > EVIDENCE_TTL_MS) continue;
      // The sender has to be a room this instance read — its record is what we are reading.
      if (!byId.has(message.fromId)) {
        this.#deferEvidence(message);
        continue;
      }
      // A create that named nothing is not dropped: it is filed with the window it ran in,
      // and matched below against whatever came into being inside it.
      if (message.toId === null && message.verb === 'create' && message.until) {
        this.pendingSpawns.push({
          ...
          queuedAt: message.queuedAt ?? at,
        });
        continue;
      }
      // The recipient only has to exist: on another host it is a peer instance's to draw.
      if (!this.knownIds.has(message.toId)) {
        this.#deferEvidence(message);
        continue;
      }
      if (message.fromId === message.toId) continue;
      if (message.verb === 'create' && !this.#couldHaveMade(message.at, message.toId)) continue;
    ...
    // The creates that named nothing, matched by time. A window stays open for a while, so a
    // worker on a slow host is still claimed when it finally lists; one nothing has claimed
    // by then is let go.
    if (this.pendingSpawns.length) {
      const stale = at - SPAWN_WINDOW_TTL_MS;
      this.pendingSpawns = this.pendingSpawns
        .filter((w) => (w.queuedAt ?? w.until) >= stale)
        .slice(-MAX_PENDING_EVIDENCE);
      // Only workspaces nobody is yet known to have made: a create that named its target
      // exactly has already been filed, and a window must not second-guess it.
      const named = new Set(this.attributed);
      for (const peers of this.spawned.values()) for (const id of peers) named.add(id);
      const unclaimed = new Map();
      for (const [id, born] of this.createdAt) if (!named.has(id)) unclaimed.set(id, born);
      for (const [toId, w] of spawnsIn(this.pendingSpawns, unclaimed)) {
        this.attributed.add(toId);
        this.#recordCall(w.fromId, { verb: 'create', workspaceId: toId });
        push('spawn', w.fromId, toId, w.text, w.replay, w.from);
      }
      // Keep bounded windows while background host metadata catches up. Attribution prevents
      // a previously matched workspace from being counted twice.
    }
    ...
    for (const agent of agents) {
      ...
      if (!before) {
        ...
        for (const terminal of agent.terminals) {
          for (const call of this.#newCalls(terminal, agent.id)) {
            if (call.workspaceId === agent.id) continue;
            if (!this.knownIds.has(call.workspaceId)) {
              this.#deferEvidence({
                fromId: agent.id,
                toId: call.workspaceId,
                verb: call.verb,
                text: call.text,
                at,
                replay: true,
            ...
            if (call.verb === 'create' && !this.#couldHaveMade(at, call.workspaceId)) continue;
      ...
      for (const terminal of agent.terminals) {
        for (const call of this.#newCalls(terminal, agent.id)) {
          ...
          // A screen call has no timestamp of its own; the tick it was first seen on is the
          // latest it can have run, which still rules out a workspace made hours before.
          if (call.verb === 'create' && !this.#couldHaveMade(at, call.workspaceId)) continue;
          this.#recordCall(agent.id, call);
      ...
      if (!before.stale && before.status !== agent.status) {
      ...
      for (const message of before.stale ? [] : agent.queued) {
  ...
  #recordCall(fromId, call) {
    ...
    if (call.verb !== 'create') return;
    if (!this.spawned.has(fromId)) this.spawned.set(fromId, new Set());
    if (!this.spawned.get(fromId).has(call.workspaceId)) this.dirty = true;
    this.spawned.get(fromId).add(call.workspaceId);
  }

  /** Whether a create run at `at` can have made `id` — see couldHaveMade. */
  #couldHaveMade(at, id) {
    return couldHaveMade(at, this.createdAt.get(id));
  }

  /** Live peers `id` shares a folder with: every other workspace under any of its tags. */
  #folderMates(id, alive) {
    const mates = new Set();
    const mine = this.tagsOf.get(id) ?? [];
    if (!mine.length) return mates;
    const tags = new Set(mine);
    for (const other of alive) {
      if (other === id) continue;
      if ((this.tagsOf.get(other) ?? []).some((t) => tags.has(t))) mates.add(other);
    }
    return mates;
  }
  ...
  /**
   * Put a workspace behind the bar by hand, or take it back out.
   *
   * Returns whether anything changed. The election is re-run against the last poll's rooms
   * straight away, so the next snapshot — which the server sends at once — already shows it;
   * the pin is saved with the rest of the state on that same tick.
   */
  pin(id, on = true) {
    if (typeof id !== 'string' || !id) return false;
    const had = this.pins.has(id);
    if (had === Boolean(on)) return false;
    if (on) this.pins.add(id);
    else this.pins.delete(id);
    this.dirty = true;
    this.#electHubs([...this.previous.values()]);
    return true;
  }
   ...
   *
   * A third shape lowers the bar to ONE inside a folder — the sessions a tag files together
   * — and is explained where it runs. A fourth is not inferred at all: a person pinned it
   * (see `pin`). There used to be a guess from any workspace NAMED `orchestrat*`, and it put
   * a session titled "Multi agent VM orchestration and scaling strategy" behind the bar of a
   * fleet it had never spoken to. A guess from a title is not evidence, and an empty bar is
   * the truer picture of a fleet whose traffic has not been seen; anyone who knows better
   * can pin. A tag is different from a title in the one way that matters: somebody put it
   * there to say these belong together.
   */
  #electHubs(agents) {
    ...
    // The folder rule. Superset lets a workspace be tagged, and every tag is a sidebar folder
    // of that name: an orchestrator that creates its workers with `--tag` has filed the whole
    // run together, and a person can drag the coordinator in beside them. That filing is a
    // statement about which sessions belong to one another, and inside it the bar for
    // election drops from two commanded peers to one — which is what puts an orchestrator of
    // a SINGLE worker behind the bar, the case the pin used to be the only answer to.
    //
    // One is enough only because of what the folder rules out. Two peers were needed so a
    // worker's report home did not elect the worker; inside a folder, the peer that CREATED a
    // mate is the coordinator beyond doubt (a worker never creates its coordinator), and a
    // peer that merely sent to one counts only while no mate has commanded it back — the
    // same guard the reporters rule uses. A folder with no traffic in it at all elects
    // nobody: the tag says these belong together, not which of them is in charge.
    for (const id of alive) {
      if (hubs.has(id)) continue;
      const mates = this.#folderMates(id, alive);
      if (!mates.size) continue;
      const made = [...(this.spawned.get(id) ?? [])].some((p) => mates.has(p));
      if (made) {
        hubs.add(id);
        continue;
      }
      const { out, into } = this.#degree(id, alive);
      const drove = out.some((p) => mates.has(p));
      const driven = into.some((p) => mates.has(p));
      if (drove && !driven) hubs.add(id);
    }

    // What a person pinned is not evidence and does not pretend to be. It is a decision, and
    // it outranks the absence of evidence: a pinned session is behind the bar whether or not
    // any of its commands were ever caught. Only a live one, though — a pin outlives the
    // session it names, and a bar patch for a workspace that no longer exists is a hole in the
    // room. The pin itself is kept, so the session is back behind the bar if it comes back.
    for (const id of this.pins) {
      if (alive.has(id)) hubs.add(id);
    }
