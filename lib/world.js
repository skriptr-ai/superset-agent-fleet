...
export class World {
   ...
   *
   * A third shape is not inferred at all: a person pinned it (see `pin`). There used to be a
   * fourth — a cold-start guess from any workspace NAMED `orchestrat*` — and it put a session
   * titled "Multi agent VM orchestration and scaling strategy" behind the bar of a fleet it had
   * never spoken to. A guess from a title is not evidence, and an empty bar is the truer picture
   * of a fleet whose traffic has not been seen; anyone who knows better can pin.
   */
  #electHubs(agents) {
