...
import { listWorkspaces, listTerminals, readTerminal, localHostId, mapLimit } from './superset.js';
...
export class World {
  ...
  async poll() {
    ...
    const reading = this.#planReads(live);
    const agents = (
      await mapLimit(live, READ_CONCURRENCY, (ws) =>
    ...
    const [logged, scripted] = await Promise.all([
      this.log.read(),
      this.transcripts.read(live.filter((w) => this.#isLocal(w.hostId))),
    ]);
