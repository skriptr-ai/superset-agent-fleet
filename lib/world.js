...
export class World {
  ...
  async poll() {
    ...
    this.createdAt = new Map(live.map((w) => [w.id, Date.parse(w.createdAt)]));
    this.tagsOf = new Map(live.map((w) => [w.id, tagsOf(w)]));
    ...
    const tasks = await lookupTasks(agents.map((a) => a.taskId));
    for (const agent of agents) this.#describe(agent, tasks.get(agent.taskId));
  ...
  #diff(agents, recorded = []) {
    ...
    for (const message of recorded) {
      ...
      if (!byId.has(message.fromId)) continue;
      ...
      if (!this.knownIds.has(message.toId)) continue;
    ...
    if (this.pendingSpawns.length) {
      ...
      this.pendingSpawns = this.pendingSpawns.filter((w) => w.replay || w.until >= stale);
      ...
      // A replayed window has done its matching against the whole live list on the tick it
      // arrived; anything it did not claim then, it never will.
      this.pendingSpawns = this.pendingSpawns.filter((w) => !w.replay);
    }
