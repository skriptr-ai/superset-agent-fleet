...
//
// And the output is whatever the orchestrator chose to print, which is not always the CLI's.
// A real run of three workers went `--json > ws-$n.json` and then printed its own one-line
// summary of each file — `{"id":…,"name":…}` — so the result named all three workspaces
// under a key this file did not read, and the coordinator stood at a table with its workers
// for an hour. Another common shape is `ws=$(superset ws create … --quiet)`, whose result is
// EMPTY: the id went into a shell variable and never reached the transcript at all. So a
// create is read three ways, best first: an id under a key that names a workspace; a bare id
// on a line of its own, which is what `--quiet` prints; and, when the output names nothing,
// the call is handed on unresolved with the time it ran, and the world matches it against
// the workspaces that came into being while it was running — see World#spawnsIn.
 ...
 *
 * `workspaces_create` is not here: the workspace it makes is named in the RESULT, not the
 * arguments, so it is parked and resolved the way a shell create is — see MCP_WORKSPACE_CREATE.
 */
const VERBS = new Map([
...
/**
 * The MCP call whose target exists only in its result: the workspace it makes. It is parked
 * like a shell create and resolved from its output, or handed on with its time window.
 */
const MCP_WORKSPACE_CREATE = 'mcp__superset__workspaces_create';
...
/**
 * The looser shapes a WORKSPACE create's output may take, and only a workspace create's: an
 * `id` key at any depth — the orchestrator's own `jq '{id, name}'` summary of the file it
 * saved the real output to — and a bare id on a line of its own, which is `--quiet`. A
 * terminal or agent create is not read this way: its `id` is the terminal's, and while the
 * world would refuse it for not being a workspace, it would still spend the cap below.
 */
const WORKSPACE_ONLY_RES = [
  new RegExp(`"id"\\s*:\\s*"(${UUID_SHAPE})"`, 'g'),
  new RegExp(`^\\s*(${UUID_SHAPE})\\s*$`, 'gm'),
];
...
/** The nouns whose `create` makes a workspace, and so may name one loosely in its output. */
const WORKSPACE_NOUNS = new Set(['workspaces', 'ws']);
 ...
 *
 * One create returns one workspace, but one Bash call is often a loop over several — the run
 * that motivated this made three in a single call, and a cap of two would have dropped the
 * third worker on the floor. A cap is still wanted: it keeps a surprising result — an error
 * listing every workspace, a verb that grew a `--all` — from turning one call into a fleet.
 */
const MAX_IDS_FROM_RESULT = 8;
...
export class Transcripts {
   ...
   * @returns {Promise<{at: number, verb: string, fromId: string, toId: string | null,
   *   until?: number, terminalId: string | null, text: string | null, replay: boolean}[]>}
   *   in time order. A create with a null `toId` is one whose output named nothing; `until`
   *   is when its result came back, and the world matches the window to a workspace.
   */
...
/**
 * What a tool result printed, as one string. Claude files a result either as a string or as
 * a list of blocks; stringifying the list would escape every quote in the JSON the CLI
 * printed, and `"workspaceId"` would then never be found in it.
 */
...
function callsIn(row, worktreePath, awaiting) {
  ...
  if (row.type === 'user') {
    ...
    for (const block of content) {
      ...
      const printed = resultText(block.content);
      const seen = new Set();
      const patterns = pending.workspace
        ? [...RESULT_WORKSPACE_RES, ...WORKSPACE_ONLY_RES]
        : RESULT_WORKSPACE_RES;
      ...
      // A create whose output named nothing still made something. It goes on with the window
      // it ran in — from the call to its answer — for the world to match against what came
      // into being meanwhile. Only a create: an unresolved send has no such tell.
      if (!seen.size && pending.verb === 'create') {
        calls.push({
          at: pending.at,
          verb: 'create',
          toId: null,
  ...
  for (const block of content) {
    ...
    if (block.name === 'Bash') {
      ...
      awaiting.set(block.id, {
        at: when,
        verb,
        text: null,
        selfId: null,
        workspace: verb === 'create' && WORKSPACE_NOUNS.has(match[1]),
      });
      continue;
    }

    if (block.name === MCP_WORKSPACE_CREATE) {
      const prompt = block.input?.prompt;
      awaiting.set(block.id, {
        at: when,
        verb: 'create',
        text: typeof prompt === 'string' && prompt ? prompt.slice(0, 2000) : null,
        selfId: null,
        workspace: true,
      });
      continue;
