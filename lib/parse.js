...
import { screenTools } from './activity.js';
...
export function classifyTerminal({ title = '', text = '' }) {
  ...
  return {
    ...
    // The tool calls of the current turn as the screen shows them — the fallback for what the
    // agent is doing when its transcript is not on this machine. See lib/activity.js.
    tools: flavor === 'shell' ? [] : screenTools(text),
  };
}
